...
onMounted(async () => {
  try {
    ...
    if (!container) {
      ...
      searchError.value = '地图容器加载失败';
      return;
    }
    console.log('容器尺寸:', { width: container.offsetWidth, height: container.offsetHeight });
    if (container.offsetWidth === 0 || container.offsetHeight === 0) {
      console.error('地图容器尺寸为0');
      searchError.value = '地图容器尺寸异常';
      return;
    }
    // 动态加载百度地图API
    if (typeof BMap === 'undefined') {
      console.log('百度地图API未加载，开始动态加载...');
      await new Promise((resolve, reject) => {
        ...
        script.src = 'https://api.map.baidu.com/api?v=3.0&ak=VcwXPliUc7qLq1R2AmlszV5S64fbRNGV&callback=initBaiduMap';
        script.async = true;
        script.defer = true;
        script.onerror = () => {
    ...
    console.log('BMap available after loading:', typeof BMap !== 'undefined');
    // 初始化百度地图实例
    ...
    searchError.value = `地图加载失败: ${error.message || error}`; // 显示具体错误信息
  }
});
